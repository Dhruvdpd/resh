...
int main() {
    ...
    while (fin >> w1) {
        ...
        if(op.count(w1) || w1=="START" || w1=="LTORG" 
           || w1=="END" || w1=="DS" || w1=="STOP" || w1=="ORIGIN"){
            w3 = "";
            w2 = w1;
        }
        else { 
        ...
        // ✅ ORIGIN (only update LC)
        if(w2=="ORIGIN") {
            fin >> w3;
            // handles both L1+3 and L1-2
            int pos = w3.find('+');
            int minusPos = w3.find('-');
            if(pos != string::npos) {
                ...
                string sym = w3.substr(0, minusPos);
                int k = stoi(w3.substr(minusPos+1));
                LC = SYM[sym] - k;
            }
            else {
                // direct ORIGIN X (no + or -)
                LC = SYM[w3];
            }
            // No IC print — as per your instruction
            continue;
        }
